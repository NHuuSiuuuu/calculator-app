...
test("AI Support is the default app tab on first load", async ({ page }) => {
  ...
  await expect(page.getByRole("tab")).toHaveText(["AI Support", "Calculator", "Todo List", "Game"]);
...
test("game tab renders Dungeon Survivor on desktop and mobile", async ({ page }) => {
  ...
  await expect(page.getByTestId("attack-effect")).toContainText("Auto fire");
  await expect(page.getByRole("button", { name: "Attack" })).not.toBeVisible();
